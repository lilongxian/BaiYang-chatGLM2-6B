...
def convert(raw_file, line_num=None):
    ...
    with open(raw_file, "r", encoding="utf-8") as file:
        dis_file = open("./explanation-based-learning-data.json", "w", encoding="utf-8")
        data = eval(file.read())
        c = 0
        for item in data:
            if line_num and c >= line_num:
                break
            if "input" in item and "instruction" in item and "output" in item and "explain" in item:
                dic = dict()
                dic["history"] = [item["input"]]
                dic["prompt"] = item["instruction"]
                dic["response"] = " EXPLAIN: " + item["explain"] + " CONCLUSION: " + item["output"]
